...
from .models import Response, State, workers, users, ServiceCatogarys, Country, City, Feedback, ServiceRequests, Contact, ServiceTracking
...
class bookservice(LoginRequiredMixin, View):
    ...
    def post(self,request,id):
        ...
        return HttpResponseRedirect('/index')  # Change this to your success page URL
...
class DeleteWorker(LoginRequiredMixin, View):
    ...
    def get(self, request, id):
        try:
            ...
            # Delete the associated User account first (this will cascade delete the worker due to OneToOneField)
            worker.admin.delete()
            return HttpResponseRedirect('/manageworker')
        except workers.DoesNotExist:
...
def ai_recommend(request):
    """View to handle AI recommendations"""
    if request.method == 'POST':
        try:
            description = request.POST.get('description', '')
            page_context = request.POST.get('page_context', '')  # Get the current page context
            ...
            # Add page context to the description
            if page_context:
                context_prompt = f"[Context: User is on the {page_context} page] {description}"
            else:
                context_prompt = description
            
            # Get recommendation
            result = ai_service.get_service_recommendation(context_prompt)
